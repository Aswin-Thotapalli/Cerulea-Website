...
const LIFECYCLE_STEPS = [
  ...
  {
    id: 'monitoring',
    title: '2. Oracle Telemetry Ingestion',
    ...
    terminal: [
      '[SYS] Polling Decentralized Oracle Network...',
      '[CMD] GET https://api.noaa.gov/weather/region_7A',
      '[SYS] Awaiting multiple node consensus for data validity...',
      '[OK] Data validated. Current rainfall metric: 3.2cm.'
    ]
  },
  {
    id: 'trigger',
    title: '3. Deterministic Trigger',
    ...
    terminal: [
      '[SYS] Evaluating telemetry against Policy ID 8842...',
      '[CMD] conditionCheck(currentRainfall < thresholdRainfall)',
      '[SYS] Condition met (3.2cm < 5.0cm).',
      '[OK] Policy threshold breached. Initiating settlement sequence.'
    ]
  },
  {
    id: 'payout',
    title: '4. Autonomous Payout',
    ...
    terminal: [
      '[SYS] Triggering Escrow Execution Vault...',
      '[CMD] Execute transfer(PolicyHolder_Wallet, 500000)',
      '[SYS] Updating ledger state and closing policy contract...',
      '[OK] Settlement complete. Zero administrative intervention.'
    ]
  }
];
...
const ANATOMY_LAYERS = [
  {
    ...
    role: 'Contract State Tracking',
    desc: 'The foundational data structure. It registers the exact parameters of the insurance agreement, including the identities of the insured and the insurer, the specific trigger thresholds, and the policy expiration block.',
    code: `contract PolicyRegistry {\n  struct Policy {\n    address insured;\n    uint256 payoutAmount;\n    uint256 triggerThreshold;\n    uint256 expirationTime;\n    bool isActive;\n  }\n  \n  mapping(uint256 => Policy) public policies;\n\n  function createPolicy(uint256 id, address client, uint256 threshold) external {\n    policies[id] = Policy(client, msg.value, threshold, block.timestamp + 365 days, true);\n  }\n}`,
    simAction: 'Simulate Policy Creation',
    simLogs: [
      "> Compiling policy parameters for Client 0x7B2...F11...",
      "> Setting trigger threshold to 500 units...",
      "> Locking provided capital into specific contract index...",
      "> Writing struct to Level 1 Storage...",
      "> [SUCCESS] Policy #8842 actively registered on network."
    ]
  },
  {
    id: 'premium',
    icon: <AccountBalanceWalletOutlinedIcon sx={{ fontSize: 32 }} />,
    title: 'LAYER 02: FINANCIAL ENGINE',
    name: 'The Premium Router',
    role: 'Payment Automation',
    desc: 'This module handles the continuous collection of insurance premiums. It can process microscopic streaming payments per second or handle traditional monthly bulk transfers, automatically voiding the policy if the user defaults.',
    code: `contract PremiumRouter {\n  mapping(uint256 => uint256) public nextPaymentDue;\n\n  function payPremium(uint256 policyId) external payable {\n    require(msg.value == requiredPremium, "Incorrect amount");\n    nextPaymentDue[policyId] = block.timestamp + 30 days;\n    _routeToTreasury(msg.value);\n  }\n\n  function checkDefault(uint256 policyId) external {\n    if(block.timestamp > nextPaymentDue[policyId]) {\n      registry.voidPolicy(policyId);\n    }\n  }\n}`,
    simAction: 'Simulate Premium Payment',
    simLogs: [
      "> Intercepting inbound stablecoin transfer...",
      "> Validating amount against Policy #8842 required premium...",
      "> Updating nextPaymentDue timestamp...",
      "> Routing capital to Insurer Treasury pool...",
      "> [SUCCESS] Premium processed. Policy remains active."
    ]
  },
  {
    id: 'oracle',
    icon: <RadarOutlinedIcon sx={{ fontSize: 32 }} />,
    title: 'LAYER 03: DATA GATEWAY',
    name: 'The Oracle Interceptor',
    role: 'Telemetry Validation',
    desc: 'Smart contracts cannot access the internet directly. This module connects to decentralized Oracle Networks (like Chainlink) to securely pull external Web2 data (like weather APIs or flight trackers) into the Web3 execution environment.',
    code: `contract OracleInterceptor {\n  function requestTelemetry(bytes32 jobId) external {\n    Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);\n    req.add("get", "https://api.aviationstack.com/v1/flights");\n    req.add("path", "data.0.flight_status");\n    sendChainlinkRequest(req, oracleFee);\n  }\n\n  function fulfill(bytes32 _requestId, string memory status) public recordChainlinkFulfillment(_requestId) {\n    latestStatus = status;\n    executionVault.evaluateTrigger(status);\n  }\n}`,
    simAction: 'Simulate Oracle Ingestion',
    simLogs: [
      "> Firing data request to decentralized Oracle nodes...",
      "> Oracles pinging external Web2 API endpoint...",
      "> Consolidating responses to prevent single-source manipulation...",
      "> Returning cryptographically signed result: 'CANCELLED'...",
      "> [SUCCESS] External state securely anchored to ledger."
    ]
  },
  {
    id: 'vault',
    icon: <SecurityOutlinedIcon sx={{ fontSize: 32 }} />,
    title: 'LAYER 04: SETTLEMENT CORE',
    name: 'The Execution Vault',
    role: 'Atomic Payouts',
    desc: 'The final executor. Once the Oracle Interceptor confirms a threshold breach, this contract instantly releases the locked escrow funds to the policyholder. It mathematically guarantees that if the event happens, the payout cannot be stopped.',
    code: `contract ExecutionVault {\n  function evaluateTrigger(uint256 policyId, uint256 currentMetric) external onlyOracle {\n    Policy memory p = registry.getPolicy(policyId);\n    require(p.isActive, "Policy voided");\n    \n    if (currentMetric >= p.triggerThreshold) {\n      registry.markClaimed(policyId);\n      payable(p.insured).transfer(p.payoutAmount);\n      emit PayoutExecuted(policyId, p.insured, p.payoutAmount);\n    }\n  }\n}`,
    simAction: 'Simulate Trigger Payout',
    simLogs: [
      "> Receiving validated telemetry from Oracle Interceptor...",
      "> Evaluating metric against Policy #8842 threshold...",
      "> [PASS] Threshold exceeded. Trigger condition met.",
      "> Unlocking 500,000 USDC from escrow vault...",
      "> [SUCCESS] Capital routed to insured wallet. Policy closed."
    ]
  }
];

export default function ParametricInsuranceUseCasePage() {
  ...
  return (
    <Box sx={{ bgcolor: '#FAFAFA', color: '#172554', overflowX: 'hidden', pb: 0 }}>
      ...
      <Box sx={{ pt: { xs: 16, md: 24 }, pb: { xs: 12, md: 16 }, position: 'relative' }}>
        <Box sx={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: '80%', height: '500px', background: 'radial-gradient(ellipse at top, rgba(2, 132, 199, 0.1), transparent 70%)', pointerEvents: 'none' }} />
        
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1.5, px: 2, py: 1, borderRadius: 2, bgcolor: 'rgba(2, 132, 199, 0.1)', border: '1px solid rgba(2, 132, 199, 0.2)', mb: 4 }}>
              <VerifiedUserOutlinedIcon sx={{ color: '#0284c7', fontSize: 20 }} />
              <Typography sx={{ color: '#0284c7', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', fontSize: '0.75rem' }}>
                Platform Use Case
              </Typography>
            </Box>
            ...
            <Typography variant="h1" sx={{ fontWeight: 800, fontSize: { xs: '3rem', md: '5rem' }, lineHeight: 1.05, letterSpacing: '-0.03em', mb: 4, mx: 'auto', maxWidth: 1000, color: '#172554' }}>
              Automate policy payouts.<br />
              <span style={{ color: '#64748B' }}>Eradicate claims processing.</span>
            </Typography>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} justifyContent="center">
              <Button 
                ...
                sx={{ 
                  bgcolor: '#0284c7', color: '#FFFFFF', px: 4, py: 2, fontSize: '1rem', fontWeight: 700, borderRadius: 2,
                  boxShadow: '0 4px 14px rgba(2, 132, 199, 0.3)',
                  '&:hover': { bgcolor: '#0369a1', transform: 'translateY(-2px)', boxShadow: '0 6px 20px rgba(2, 132, 199, 0.4)' },
                  transition: 'all 0.2s'
                }}
              >
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: { xs: 6, lg: 8 } }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>01.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Deterministic Execution</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>Smart contracts execute strictly as coded. If a predefined condition is met, the payout triggers automatically. This completely removes subjective human interpretation from the claims process.</Typography>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>02.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Oracle Ingestion</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>Connect policies to decentralized oracle networks. This allows the blockchain to securely access real-world data like seismology reports, flight radars, and agricultural weather telemetry.</Typography>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>03.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Automated Escrow</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>Capital is pre-funded into programmatic escrow vaults. The network mathematically guarantees that the insurer has the exact liquidity required to cover the policy if the event occurs.</Typography>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>04.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Trustless Verification</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>Because the data payload and the payout logic are both permanently anchored to a public or consortium ledger, third-party auditors can instantly verify that the claim was settled fairly.</Typography>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>05.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Micro-Premiums</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>High-throughput consensus architecture enables micro-insurance models. Users can purchase highly specific policies for fractions of a cent, allowing coverage for localized, short-term risks.</Typography>
            </Box>

            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <Typography sx={{ color: '#0284c7', fontWeight: 900, fontSize: '1.5rem', mb: 1 }}>06.</Typography>
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1 }}>Algorithmic Actuaries</Typography>
              <Typography sx={{ color: '#475569', fontSize: '0.95rem', lineHeight: 1.7, textAlign: 'justify' }}>Integrate complex data models directly into the underwriting logic. Premium pricing updates dynamically in real-time based on live environmental risk factors ingested by the network.</Typography>
            </Box>
          </Box>
        </motion.div>
      </Container>
        ...
        <Container maxWidth="xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
            <Box sx={{ textAlign: 'center', mb: 10 }}>
              <Typography sx={{ fontSize: '2.5rem', fontWeight: 800, mb: 2, letterSpacing: '-0.02em', color: '#172554' }}>The Parametric Lifecycle</Typography>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, gap: 6, alignItems: 'stretch' }}>
                  ...
                  {LIFECYCLE_STEPS.map((step) => (
                    <Box 
                      ...
                      sx={{ 
                        ...
                        cursor: 'pointer',
                        border: '1px solid',
                        borderColor: activeStep.id === step.id ? 'rgba(2, 132, 199, 0.4)' : '#E2E8F0',
                        bgcolor: activeStep.id === step.id ? 'rgba(2, 132, 199, 0.05)' : '#FFFFFF',
                        transition: 'all 0.2s ease',
                        '&:hover': { bgcolor: activeStep.id === step.id ? 'rgba(2, 132, 199, 0.08)' : '#F8FAFC' }
                      }}
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                        <CheckCircleOutlineOutlinedIcon sx={{ color: activeStep.id === step.id ? '#0284c7' : '#CBD5E1', fontSize: 24 }} />
              ...
              <Box sx={{ 
                ...
                <Box sx={{ display: 'flex', gap: 1, mb: 4, borderBottom: '1px solid #1e293b', pb: 2 }}>
                  ...
                  <Typography sx={{ color: '#64748B', fontSize: '0.75rem', ml: 2, fontFamily: 'monospace' }}>cerulea_policy_engine.log</Typography>
                </Box>
          ...
          <Box sx={{ textAlign: 'center', mb: 8 }}>
            ...
            {!isDeconstructed ? (
              <Button 
                ...
                sx={{ 
                  borderColor: '#0284c7', color: '#0284c7', px: 4, py: 1.5, fontSize: '1rem', fontWeight: 700, borderRadius: 2,
                  '&:hover': { bgcolor: 'rgba(2, 132, 199, 0.05)', borderColor: '#0369a1' }
                }}
              >
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, gap: 4, alignItems: 'stretch' }}>
                  ...
                  <Box sx={{ width: { xs: '100%', lg: '350px' }, flexShrink: 0 }}>
                    <Stack spacing={2} sx={{ position: 'relative' }}>
                      ...
                      <Box sx={{ position: 'absolute', top: 30, bottom: 30, left: 34, width: '2px', bgcolor: 'rgba(2, 132, 199, 0.2)', zIndex: 0 }} />
                        ...
                        >
                          <Box 
                            ...
                            sx={{ 
                              ...
                              border: '1px solid',
                              borderColor: activeLayer.id === layer.id ? '#0284c7' : '#E2E8F0',
                              borderRadius: 3, 
                              cursor: 'pointer',
                              transition: 'all 0.2s ease',
                              boxShadow: activeLayer.id === layer.id ? '0 10px 30px rgba(2, 132, 199, 0.15)' : 'none',
                              transform: activeLayer.id === layer.id ? 'translateX(10px)' : 'none'
                            }}
                          >
                            <Box sx={{ width: 48, height: 48, borderRadius: 2, bgcolor: activeLayer.id === layer.id ? 'rgba(2, 132, 199, 0.1)' : '#FFFFFF', border: '1px solid', borderColor: activeLayer.id === layer.id ? 'rgba(2, 132, 199, 0.3)' : '#E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <Box sx={{ color: activeLayer.id === layer.id ? '#0284c7' : '#94A3B8', display: 'flex' }}>{layer.icon}</Box>
                            </Box>
                            <Box>
                              <Typography sx={{ color: activeLayer.id === layer.id ? '#0284c7' : '#64748B', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', mb: 0.5, textTransform: 'uppercase' }}>
                                {layer.role}
                              </Typography>
                      ...
                      {['architecture', 'code', 'simulate'].map((tab) => (
                        <Box 
                          ...
                          sx={{ 
                            ...
                            borderColor: activeTab === tab ? '#0284c7' : 'transparent',
                        ...
                        >
                          ...
                          {activeTab === 'architecture' && (
                            <Box>
                              ...
                              <Box sx={{ p: 3, bgcolor: 'rgba(2, 132, 199, 0.05)', borderRadius: 2, border: '1px solid rgba(2, 132, 199, 0.2)' }}>
                                <Typography sx={{ color: '#0284c7', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.1em', mb: 1 }}>PLATFORM FUNCTION</Typography>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          ...
          <Stack spacing={4}>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, p: 4, bgcolor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 4, alignItems: { xs: 'flex-start', md: 'center' }, gap: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
              <Box sx={{ width: 80, height: 80, borderRadius: 3, bgcolor: 'rgba(2, 132, 199, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <AgricultureOutlinedIcon sx={{ fontSize: 40, color: '#0284c7' }} />
              </Box>
              <Box sx={{ flexGrow: 1 }}>
                <Typography sx={{ fontSize: '1.5rem', fontWeight: 700, color: '#172554', mb: 1 }}>Agriculture & Crop Protection</Typography>
              ...
              <Box sx={{ flexShrink: 0, minWidth: 220, pl: { md: 4 }, borderLeft: { md: '1px solid #E2E8F0' } }}>
                ...
                <Stack spacing={1.5}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Drought & Flood Protection</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Yield Failure Escrow</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Micro-Farmer Insurance</Typography></Box>
                </Stack>
              </Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, p: 4, bgcolor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 4, alignItems: { xs: 'flex-start', md: 'center' }, gap: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
              <Box sx={{ width: 80, height: 80, borderRadius: 3, bgcolor: 'rgba(2, 132, 199, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <FlightTakeoffOutlinedIcon sx={{ fontSize: 40, color: '#0284c7' }} />
              </Box>
              <Box sx={{ flexGrow: 1 }}>
                <Typography sx={{ fontSize: '1.5rem', fontWeight: 700, color: '#172554', mb: 1 }}>Travel & Aviation Risk</Typography>
                <Typography sx={{ color: '#475569', fontSize: '1rem', lineHeight: 1.6, textAlign: 'justify' }}>
              ...
              <Box sx={{ flexShrink: 0, minWidth: 220, pl: { md: 4 }, borderLeft: { md: '1px solid #E2E8F0' } }}>
                <Typography sx={{ fontSize: '0.75rem', color: '#64748B', fontWeight: 700, letterSpacing: '0.05em', mb: 2 }}>KEY DEPLOYMENTS</Typography>
                <Stack spacing={1.5}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Flight Delay Payouts</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Lost Baggage Tracking</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Travel Interruption Shield</Typography></Box>
                </Stack>
              </Box>
            </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, p: 4, bgcolor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 4, alignItems: { xs: 'flex-start', md: 'center' }, gap: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
              <Box sx={{ width: 80, height: 80, borderRadius: 3, bgcolor: 'rgba(2, 132, 199, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <LocalShippingOutlinedIcon sx={{ fontSize: 40, color: '#0284c7' }} />
              </Box>
              <Box flexGrow={1}>
              ...
              <Box sx={{ flexShrink: 0, minWidth: 220, pl: { md: 4 }, borderLeft: { md: '1px solid #E2E8F0' } }}>
                <Typography sx={{ fontSize: '0.75rem', color: '#64748B', fontWeight: 700, letterSpacing: '0.05em', mb: 2 }}>KEY DEPLOYMENTS</Typography>
                <Stack spacing={1.5}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Cold-Chain Failures</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Port Congestion Delays</Typography></Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><ArrowForwardIcon sx={{ fontSize: 14, color: '#0284c7' }}/><Typography sx={{ fontSize: '0.9rem', color: '#172554', fontWeight: 600 }}>Maritime Incident Coverage</Typography></Box>
                </Stack>
              </Box>
            </Box>
          </Stack>
        ...
        <Container maxWidth="xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
            ...
            <Stack spacing={10}>
              ...
              <Box>
                <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1, textAlign: 'center' }}>Track A: Enterprise Underwriting Bridging</Typography>
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, alignItems: 'center', justifyContent: 'center', gap: 2, position: 'relative' }}>
                  ...
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    ...
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 700, color: '#172554', mb: 0.5 }}>Legacy Insurer Core</Typography>
                    <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>Guidewire / Actuarial DB</Typography>
                  </Box>
                  ...
                  {/* Node 2 */}
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    <Box sx={{ width: 70, height: 70, mx: 'auto', bgcolor: 'rgba(2, 132, 199, 0.05)', border: '2px solid #0284c7', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2, boxShadow: '0 4px 14px rgba(2, 132, 199, 0.15)' }}>
                      <ApiOutlinedIcon sx={{ color: '#0284c7', fontSize: 32 }} />
                    </Box>
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 700, color: '#172554', mb: 0.5 }}>Cerulea API Gateway</Typography>
                    <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>Policy Parameter Validation</Typography>
                  </Box>

                  {/* Arrow 2 */}
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 140 }}>
                    <Typography sx={{ fontSize: '0.65rem', color: '#0284c7', fontWeight: 700, mb: 0.5, letterSpacing: '0.05em' }}>WASM COMPILATION</Typography>
                    <ArrowForwardIcon sx={{ color: '#0284c7', fontSize: 24, transform: { xs: 'rotate(90deg)', lg: 'none' } }} />
                  </Box>
                  ...
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    ...
                    <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>Sovereign Escrow State</Typography>
                  </Box>
              ...
              <Box>
                <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: '#172554', mb: 1, textAlign: 'center' }}>Track B: Native Oracle Execution</Typography>
                <Typography sx={{ color: '#475569', fontSize: '0.95rem', mb: 6, lineHeight: 1.6, textAlign: 'center', maxWidth: 600, mx: 'auto' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, alignItems: 'center', justifyContent: 'center', gap: 2, position: 'relative' }}>
                  ...
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    ...
                    <Typography sx={{ fontSize: '0.85rem', fontWeight: 700, color: '#172554', mb: 0.5 }}>External Event Data</Typography>
                    <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>Weather APIs & Flight Logs</Typography>
                  </Box>

                  {/* Arrow 1 */}
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 140 }}>
                    <Typography sx={{ fontSize: '0.65rem', color: '#64748B', fontWeight: 700, mb: 0.5, letterSpacing: '0.05em' }}>JSON-RPC PULL</Typography>
                    <ArrowForwardIcon sx={{ color: '#64748B', fontSize: 24, transform: { xs: 'rotate(90deg)', lg: 'none' } }} />
                  </Box>
                  ...
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    <Box sx={{ width: 70, height: 70, mx: 'auto', bgcolor: 'rgba(37, 99, 235, 0.05)', border: '2px solid #2563eb', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2, boxShadow: '0 4px 14px rgba(37, 99, 235, 0.15)' }}>
                      <RadarOutlinedIcon sx={{ color: '#2563eb', fontSize: 32 }} />
                    </Box>
                  ...
                  <Box sx={{ textAlign: 'center', width: 220 }}>
                    ...
                    <Typography sx={{ fontSize: '0.75rem', color: '#475569' }}>Trigger & Payout Logic</Typography>
                  </Box>
          ...
          <Box sx={{ p: { xs: 4, md: 6 }, bgcolor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 4, boxShadow: '0 20px 40px rgba(0,0,0,0.05)' }}>
            <Grid container spacing={8} alignItems="center">
              <Grid size={{ xs: 12, md: 6 }}>
                <Typography sx={{ color: '#172554', fontWeight: 600, mb: 1 }}>Required Actuarial Conditions & Oracles</Typography>
                <Typography sx={{ color: '#0284c7', fontSize: '3rem', fontWeight: 800, mb: 4 }}>{complexity} Rules</Typography>
                
                <Slider
                  value={complexity} onChange={handleComplexityChange} min={10} max={200} step={10}
                  sx={{ color: '#0284c7', height: 8, '& .MuiSlider-thumb': { height: 24, width: 24, backgroundColor: '#fff', border: '2px solid currentColor' } }}
                />
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Stack spacing={3}>
                  ...
                  <Box sx={{ p: 3, bgcolor: 'rgba(2, 132, 199, 0.05)', border: '1px solid rgba(2, 132, 199, 0.2)', borderRadius: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box>
                      <Typography sx={{ fontSize: '0.75rem', color: '#0284c7', fontWeight: 700, mb: 0.5, letterSpacing: '0.05em' }}>CERULEA EXECUTION</Typography>
