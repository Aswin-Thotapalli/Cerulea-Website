...
export default function AdminControls({ postSlug, showNewPostButton }: Props) {
  // Without Supabase env the admin UI can't function; render nothing.
  if (!SUPABASE_CONFIGURED) return null
  return <AdminControlsInner postSlug={postSlug} showNewPostButton={showNewPostButton} />
}

function AdminControlsInner({ postSlug, showNewPostButton }: Props) {
  ...
  return (
    ...
    >
      {!session && (
        <button onClick={handleLogin} style={btnStyle("#0c1e45", "#fff")}>
          Login
        </button>
      )}
      ...
      {session && !isAdmin && (
        ...
        >
          ...
          <button
            ...
            style={{ ...btnStyle("#be123c", "#fff"), fontSize: 12, padding: "6px 12px" }}
          >
            Sign out
          </button>
        </div>
      ...
      {isAdmin && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
            {showNewPostButton && (
              <button onClick={() => router.push("/blog/manage")} style={btnStyle("#0c1e45", "#fff")}>
                ⚙ Manage
              </button>
            )}
            {showNewPostButton && (
            ...
            {postSlug && (
              <>
                <button onClick={() => router.push(`/blog/edit/${postSlug}`)} style={btnStyle("#0c1e45", "#fff")}>
                  ✏ Edit post
                </button>
...
function btnStyle(bg: string, color: string = "#fff"): React.CSSProperties {
  return {
    ...
    fontWeight: 700,
    cursor: "pointer",
    boxShadow: "0 4px 14px rgba(12,30,69,0.15)",
    whiteSpace: "nowrap",
  }
}
