    ...
    >
      ...
      >
          ...
          onChange={(e) => {
            ...
            if (v === 0) editor.chain().focus().setParagraph().run()
              ...
              editor
                .chain()
                .focus()
                .setHeading({ level: v as 1 | 2 | 3 | 4 | 5 | 6 })
                .run()
          }}
          style={{
            padding: "4px 8px",
        ...
        <label title="Text color" style={{ display: "flex", alignItems: "center", gap: 2, cursor: "pointer", fontSize: 12, color: "#374151", padding: "0 4px" }}>
          A
          <input type="color" defaultValue="#000000" onChange={(e) => editor.chain().focus().setColor(e.target.value).run()} style={{ width: 16, height: 16, border: "none", cursor: "pointer", padding: 0 }} />
        </label>
        <label title="Highlight" style={{ display: "flex", alignItems: "center", gap: 2, cursor: "pointer", fontSize: 12, color: "#374151", padding: "0 4px" }}>
          ▌
          <input type="color" defaultValue="#fef08a" onChange={(e) => editor.chain().focus().setHighlight({ color: e.target.value }).run()} style={{ width: 16, height: 16, border: "none", cursor: "pointer", padding: 0 }} />
        </label>
        ...
        <input ref={fileInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadImage(f); e.target.value = "" }} />
      ...
      <style>{`
        ...
        .ProseMirror h1 { font-size:2rem; font-weight:800; margin:1.5rem 0 .75rem; color:#0c1e45; letter-spacing:-.02em; }
        .ProseMirror h2 { font-size:1.5rem; font-weight:700; margin:1.5rem 0 .75rem; color:#0c1e45; }
        .ProseMirror h3 { font-size:1.25rem; font-weight:700; margin:1.25rem 0 .5rem; color:#0c1e45; }
        .ProseMirror h4,.ProseMirror h5,.ProseMirror h6 { font-size:1rem; font-weight:700; margin:1rem 0 .5rem; color:#0c1e45; }
        ...
        .ProseMirror pre { background:#0c1e45; color:#e2e8f0; border-radius:8px; padding:1rem 1.25rem; margin:1.5rem 0; overflow-x:auto; }
        ...
        .ProseMirror th { background:#F8FAFC; font-weight:700; color:#0c1e45; }
